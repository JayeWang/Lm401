...
function expt() {
  var oHtml = document.getElementById("tableId").outerHTML;
...
`;
  ...
  index = month - 1 + "月" + select.value;
  // 利用URL.createObjectURL()方法为a元素生成blob URL
  oA.href = URL.createObjectURL(excelBlob);
  // 给文件命名
  oA.download = index + "周签到信息报表.xls";
  // 模拟点击
  oA.click();
}
function getInfo(para) {
  ...
  var infoName = para.parentNode.parentNode.getElementsByTagName("td")[0]
    .innerText;
  console.log(infoName); //test
  var _info = document.getElementById("infoName");
  _info.innerHTML = infoName;
  var ajax = $.ajax({
    type: "get",
    url: localStorage.testurl + "/admin/getOneTime",
    data: {
      startDate: start,
      endDate: end,
      userName: infoName
    ...
    success: function(data1) {
      ...
      if (data1.code == 0) {
        //////////////
        var list = document.getElementById("listr");
        list.innerHTML =
          "<tr><td>姓名</td><td>签到日期</td><td>签到时间</td><td>签退时间</td><td>签到总时长</td><td>备注</td></tr>";
        //  list.innerHTML = 'null';
...
function getAll() {
  ...
  var ajax = $.ajax({
    type: "get",
    url: localStorage.testurl + "/admin/getAllTime",
    data: {
      startDate: start,
      endDate: end
    },
    ...
    success: function(data1) {
      ...
      if (data1.code == 0) {
        ...
        list.innerHTML =
          "<tr><td>姓名</td><td>签到次数</td><td>签到平均时长</td><td>签到总时长</td><td>备注</td></tr>";
        for (var i in data1.data) {
          var tr = document.createElement("tr");
          tr.innerHTML =
            "<td>" +
            data1.data[i].userName +
            "</td>" +
            "<td>" +
            data1.data[i].timeFre +
            "</td>" +
            "<td>" +
            data1.data[i].ahours +
            "H" +
            data1.data[i].aminutes +
            "M" +
            "</td>" +
            "<td>" +
            data1.data[i].totalHours +
            "H" +
            data1.data[i].totalMinutes +
            "M" +
            "</td>";
          if (data1.data[i].isQualified == "N") {
            tr.innerHTML +=
              '<td><input class="btn btn-danger btn-lg" type="button" value="不合格" data-toggle="modal" data-target="#infoModal" onclick="getInfo(this)"></td>';
          } else {
            tr.innerHTML +=
              '<td><input class="btn btn-success btn-lg" type="button" value="合格" data-toggle="modal" data-target="#infoModal" onclick="getInfo(this)"></td>';
          }
          list.appendChild(tr);
        }
      } else {
